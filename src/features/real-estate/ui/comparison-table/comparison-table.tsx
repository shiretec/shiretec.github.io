...
export const ComparisonTable: React.FC<ComparisonTableProps> = ({
  ...
  const lowestPaybackPeriod = Math.min(...properties.filter(p => (p.paybackPeriod || 0) > 0).map(p => p.paybackPeriod || Infinity));

  return (
    <Box mt={8}>
      <Heading size="md" mb={4}>Сравнение объектов</Heading>
      ...
      <Grid templateColumns={`auto repeat(${properties.length}, 1fr)`} gap={4}>
        ...
        {properties.map((property) => (
          <GridItem key={property.id}>
            <Box borderWidth="1px" borderRadius="md" overflow="hidden">
              <Box bg="gray.100" p={2}>
                <Heading size="sm">{property.name || 'Объект недвижимости'}</Heading>
        ...
        {properties.map((property) => (
          <GridItem key={property.id}>
            {property.propertyType === 'apartment' ? 'Квартира' : 
             ...
             property.propertyType === 'commercial' ? 'Коммерческая недвижимость' : 
        ...
        {properties.map((property) => (
          <GridItem key={property.id}>{property.initialInvestment.toLocaleString()} ₽</GridItem>
        ))}
        
        <GridItem fontWeight="bold">Первоначальный взнос</GridItem>
        {properties.map((property) => (
          <GridItem key={property.id}>{property.downPayment.toLocaleString()} ₽</GridItem>
        ))}
        ...
        {properties.map((property) => (
          <GridItem key={property.id}>{property.propertyTax.toLocaleString()} ₽/год</GridItem>
        ))}
        
        <GridItem fontWeight="bold">Страховка</GridItem>
        {properties.map((property) => (
          <GridItem key={property.id}>{property.insurance.toLocaleString()} ₽/год</GridItem>
        ))}
        
        <GridItem fontWeight="bold">Коммунальные услуги</GridItem>
        {properties.map((property) => (
          <GridItem key={property.id}>{property.utilities.toLocaleString()} ₽/мес</GridItem>
        ))}
        
        <GridItem fontWeight="bold">Обслуживание</GridItem>
        {properties.map((property) => (
          <GridItem key={property.id}>{property.maintenance.toLocaleString()} ₽/мес</GridItem>
        ))}
        
        <GridItem fontWeight="bold">Управление недвижимостью</GridItem>
        {properties.map((property) => (
          <GridItem key={property.id}>{property.managementFees.toLocaleString()} ₽/мес</GridItem>
        ))}
        ...
        {properties.map((property) => (
          <GridItem key={property.id}>{property.monthlyRent.toLocaleString()} ₽/мес</GridItem>
        ))}
        ...
        {properties.map((property) => (
          <GridItem 
            key={property.id} 
            bg={(property.cashFlow || 0) === highestCashFlow && highestCashFlow > 0 ? 'green.100' : undefined}
            fontWeight={(property.cashFlow || 0) === highestCashFlow && highestCashFlow > 0 ? 'bold' : 'normal'}
          >
        ...
        {properties.map((property) => (
          <GridItem 
            key={property.id}
            bg={(property.roi || 0) === highestROI && highestROI > 0 ? 'green.100' : undefined}
            fontWeight={(property.roi || 0) === highestROI && highestROI > 0 ? 'bold' : 'normal'}
          >
        ...
        {properties.map((property) => (
          <GridItem 
            key={property.id}
            bg={(property.paybackPeriod || 0) === lowestPaybackPeriod && lowestPaybackPeriod < Infinity ? 'green.100' : undefined}
            fontWeight={(property.paybackPeriod || 0) === lowestPaybackPeriod && lowestPaybackPeriod < Infinity ? 'bold' : 'normal'}
          >
        ...
        {properties.map((property) => (
          <GridItem key={property.id}>{(property.cashOnCash || 0).toFixed(2)}%</GridItem>
        ))}
        
        <GridItem fontWeight="bold">Капитализация</GridItem>
        {properties.map((property) => (
          <GridItem key={property.id}>{(property.capRate || 0).toFixed(2)}%</GridItem>
        ))}
      </Grid>
    </Box>
  );
